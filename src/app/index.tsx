...
import { apiUrl, Library, readLibrary, RouteResult, SavedRoute, searchRoutes, storageKey, validate } from '@/lib/routes';
...
const distance = (meters: number) => `${(meters / 1000).toFixed(1).replace('.', ',')} km`;
export default function HomeScreen() {
  ...
  async function navigate(apple = false) {
    if (!result) return;
    const destination = result.legs[completed]?.address;
    if (!destination) return;
    const departure = completed === 0 ? origin : result.legs[completed - 1].address;
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    {tab === 'Planejar' ? <>
      ...
      {!apiUrl && <View style={styles.notice}><Text style={styles.noticeTitle}>Seus endereços já podem ficar organizados</Text><Text style={styles.noticeText}>Salve seus favoritos agora. A busca da sequência mais rápida estará disponível após a configuração do serviço de mapas.</Text></View>}
      {result && <View style={styles.card}><Text style={styles.eyebrow}>SUA SEQUÊNCIA ESTÁ PRONTA</Text><Text style={styles.sectionTitle}>{duration(result.seconds)} <Text style={styles.muted}>· {distance(result.meters)}</Text></Text><Text style={styles.muted}>Menor tempo estimado entre as paradas, sem trânsito em tempo real. Estimativas: Google Maps.</Text>{result.legs.map((leg, index) => <View key={index} style={[styles.leg, index < completed && { opacity: 0.5 }]}><View style={styles.number}><Text style={styles.numberText}>{index < completed ? '✓' : index + 1}</Text></View><View style={{ flex: 1 }}><Text style={styles.address}>{leg.address}</Text><Text style={styles.muted}>{index < completed ? 'Concluída' : index === completed ? 'Próxima parada · ' : ''}{index >= completed && `${duration(leg.seconds)} · ${distance(leg.meters)}`}</Text></View></View>)}<Button title="Abrir próxima parada no Google Maps" onPress={() => navigate()} />{Platform.OS === 'ios' && <Button title="Abrir no Mapas da Apple" secondary onPress={() => navigate(true)} />}<Button title={completed + 1 === result.legs.length ? '✓  Concluir viagem' : '✓  Marcar parada como concluída'} secondary disabled={saving || !ready} onPress={finishStop} /></View>}
    ...
    <Text style={styles.footer}>GH ROTAS  ·  Feito para seguir em frente.</Text><Text style={styles.footnote}>Favoritos e histórico ficam no dispositivo. Ao buscar rotas, os endereços são enviados ao serviço de mapas. Ao abrir a navegação, a partida e o destino são compartilhados com o mapa escolhido.</Text>
